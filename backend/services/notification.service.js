...
/**
 * Match determines when the confidence threshold is crossed; Notification only creates the notification.
 * Integration function for Navika's Match module.
 * 
...
async function createMatchNotification(matchId, studentId, message) {
  ...
  // Check if referenced Match exists
  const [matchRows] = await pool.execute(
    'SELECT MatchID FROM `MATCH` WHERE MatchID = ?',
    [matchId]
  );
